...
These are the images to include in the GitHub repo, where each one comes from in the notebook, and the filenames used above. All six are included in this download under `images/`, already extracted and renamed from your actual notebook output.

| Image | Source in notebook | Suggested placement in README |
|---|---|---|
| `training_curves_best_model.png` | Cell 6 ("AUTO LOOP: E → F → G"), Experiment F block — accuracy/loss plot | Under **Results**, right after the results table, as "Best model training curves" |
| `sample_predictions.png` | Cell 6, Experiment F block — 5×5 grid of test predictions with confidence | Under **Results**, right after the training curves |
| `experiment_comparison_accuracy_loss.png` | Cell 6, final comparison section — val accuracy/loss for E, F, G overlaid | Under **Results**, above the "Optimizer comparison" table |
| `experiment_comparison_speed_tradeoff.png` | Cell 6, final comparison section — accuracy vs seconds/epoch bar chart | Same spot, right after the line-plot comparison |
| `optimizer_comparison_accuracy_loss.png` | Cell 5 ("OPTIMIZER COMPARISON") — val accuracy/loss for Adam/AdamW/SGD/RMSprop | Under **Results**, above the optimizer comparison table |
| `optimizer_speed_tradeoff.png` | Cell 5, same section — accuracy vs seconds/epoch bar chart | Same spot, right after the optimizer line-plot comparison |

Embed them like this once they're pushed to the repo:

```markdown
![Training curves](images/training_curves_best_model.png)
![Sample predictions](images/sample_predictions.png)
![Experiment comparison](images/experiment_comparison_accuracy_loss.png)
![Speed tradeoff](images/experiment_comparison_speed_tradeoff.png)
![Optimizer comparison](images/optimizer_comparison_accuracy_loss.png)
![Optimizer speed tradeoff](images/optimizer_speed_tradeoff.png)
```
...
*Add your name, GitHub profile link, and any contact/portfolio links here.*
